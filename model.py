...
Model source taken from the official PromptIR-main repository.
"""
...
class PromptIR(nn.Module):
    def __init__(
        ...
        if self.decoder:
            ...
            # degradation-aware prompt here helps restore fine edge/streak detail.
            # prompt_dim=96 → cat dim = 96+96 = 192 = 4*dim,
            # perfectly aligned with the architecture's dim multiples
            self.prompt0 = PromptGenBlock(
            ...
            self.reduce_noise_level0 = nn.Conv2d(
    ...
    def forward(self, inp_img, noise_emb=None):
        ...
        # Decoder Level 3 (with optional prompt)
        if self.decoder:
        ...
        # Decoder Level 2 (with optional prompt)
        if self.decoder:
        ...
        # Decoder Level 1 (with optional prompt)
        if self.decoder:
        ...
        # Full-resolution prompt interaction (Level 0) — before refinement
        # Provides degradation-specific guidance at the highest spatial resolution,
        # targeting fine structures like rain streaks and snow particle edges.
        if self.decoder:
            dec0_param = self.prompt0(out_dec_level1)
            out_dec_level1 = torch.cat([out_dec_level1, dec0_param], 1)
            out_dec_level1 = self.noise_level0(out_dec_level1)
            out_dec_level1 = self.reduce_noise_level0(out_dec_level1)
